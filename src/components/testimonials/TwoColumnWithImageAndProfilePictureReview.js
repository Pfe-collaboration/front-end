...
const HighlightedText = tw.span`bg-primary-500 text-white px-4 transform -skew-x-12 inline-block italic rounded-full  `;

export default ({
  subheading = "",
  heading = "My collaborations",
  description = ".",
  testimonials,
  textOnLeft = false,
}) => {
  ...
  const imgs = (product) => {
    if (product === "pistachio") {
      return "https://cdn.shopify.com/s/files/1/0437/8953/files/quality_pistachios.png?v=1637968526";
    } else {
      return "https://static.vecteezy.com/system/resources/thumbnails/002/574/782/small/almonds-falling-onto-wooden-surface-in-super-slow-motion-shot-on-phantom-flex-4k-high-speed-camera-video.jpg";
    }
  };
  ...
  return testimonials.length > 0 ? (
                  ...
                  {testimonials.map((testimonial, index) => (
                    <TestimonialText key={index}>
                      <HighlightedText>
                        <HeadingInfo
                          tw="hidden lg:block"
                          description={testimonial.ProductType}
                        />
                      </HighlightedText>
                      <QuoteContainer>
                        <Quote>
                      ...
                      <CustomerInfo>
                        <CustomerTextInfo>
                          <CustomerName>
                            Requested Quantity : {testimonial.RequestedQT} kg
                          </CustomerName>
                          <CustomerTitle>
                            Available : {testimonial.availableQT} kg
                          </CustomerTitle>
                        </CustomerTextInfo>
                      </CustomerInfo>
                    </TestimonialText>
    ...
    <Quote tw="mt-12">
      You don't have any collaborations fill in the form and ask for as much
      quantity of bio pistachio or almond as you want
    </Quote>
  );
};
